...
(function($) {
	...
	function drawCalControl (target, opt) {
		...
		$(window).on('load',  //please note that $().load(function(){}) is depreciated, so here need to change to .on('load', function(){})
			function () {
				$('.jCal', target).each(
					function () {
						var width = $(this).parent().width() - ( $('.left', this).width() || 0 ) - ( $('.right', this).width() || 0 );
	...
	function reSelectDates (target, day, days, opt) {
		...
		for (var fC = false, di = 0, dC = days; di < dC; di++) {
			...
			if ( dF.length > 0 ) {
				dF.stop().addClass('selectedDay');   /* Here is the selected day shadow */
				fC = true;
			}
